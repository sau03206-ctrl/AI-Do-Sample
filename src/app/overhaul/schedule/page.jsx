...
import { Card, Button, Icon, FieldChip, EmptyState } from "@/components/overhaul/ui";
import { useStore } from "@/lib/overhaul/store";
import { taskProgress } from "@/lib/overhaul/progress";
import { taskSchedule, plannedProgressOn, plannedOverall, diffDays, toDate, ymd, addDays } from "@/lib/overhaul/schedule";
    ...
    return (
      <Card lift={false}>
        <EmptyState
          ...
          action={<Button onClick={() => router.push("/overhaul/upload")}>업로드 분석으로</Button>}
        />
      </Card>
    );
  }
                    ...
                    {isOpen &&
                      g.list.map((r) => (
                        <div
                          ...
                          onClick={() => router.push(`/overhaul/entry?task=${r.task.id}`)}
                        >
