...
import { useStore, ROLES } from "@/lib/overhaul/store";

const NAV = [
  { to: "/overhaul", icon: "dashboard", label: "대시보드", end: true },
  { to: "/overhaul/upload", icon: "analytics", label: "업로드 분석" },
  { to: "/overhaul/tasks", icon: "format_list_bulleted", label: "작업 관리" },
  { to: "/overhaul/schedule", icon: "calendar_view_week", label: "공정표" },
  { to: "/overhaul/entry", icon: "edit_note", label: "실적 입력" },
  { to: "/overhaul/reports", icon: "summarize", label: "보고서" },
];

function isNavActive(pathname, n) {
  return n.end ? pathname === n.to : pathname.startsWith(n.to);
}
...
function MobileLink({ n }) {
  ...
  const active = isNavActive(pathname, n);
  return (
    <Link href={n.to} className={`flex flex-col items-center gap-0.5 ${active ? "text-primary" : "text-on-surface-variant"}`}>
...
function OverhaulNav({ children }) {
  const { roleInfo, state } = useStore();
  const pathname = usePathname();
    ...
    <div className="flex min-h-screen overflow-x-hidden">
      ...
      <aside className="hidden md:flex flex-col p-4 gap-2 h-screen fixed left-0 top-0 w-[280px] bg-surface-container border-r border-border-subtle z-50">
          ...
          {NAV.map((n) => {
            ...
            return (
              <Link
                ...
                href={n.to}
                className={`${linkBase} ${
                  active ? "bg-primary-container text-on-primary font-bold translate-x-1" : "text-on-surface-variant hover:bg-surface-container-high"
                }`}
              >
                <Icon name={n.icon} />
                <span>{n.label}</span>
        ...
        <div className="mt-auto pt-4 border-t border-border-subtle flex flex-col gap-1">
          <button
            onClick={() => router.push("/overhaul/upload")}
          ...
          <div className="flex items-center gap-3 px-4 py-2 rounded-lg text-on-surface-variant">
            ...
            <span className="text-sm">
              현재 권한: <b className="text-on-surface">{roleInfo.label}</b>
            </span>
          </div>
        </div>
      </aside>
...
export default function OverhaulShell({ children }) {
  ...
  return <OverhaulNav>{children}</OverhaulNav>;
}
