...
import { Card, ProgressRing, ProgressBar, StatusChip, Icon, Button } from "@/components/overhaul/ui";
import { useStore } from "@/lib/overhaul/store";
import {
  overallProgress,
  progressByField,
  progressByEquipment,
  ...
  missingEntries,
  fieldColor,
} from "@/lib/overhaul/progress";
import { plannedOverall, scheduleDelayTasks } from "@/lib/overhaul/schedule";
import { exportTasksToExcel } from "@/lib/overhaul/exporter";
    ...
    <>
      ...
      {alert && (
        <div className="bg-error-container text-on-error-container p-4 rounded-xl flex flex-col sm:flex-row sm:items-center justify-between gap-3 border border-error/20">
          ...
          <Button variant="danger" className="shrink-0" onClick={() => router.push("/overhaul/entry")}>
            지금 입력하기
          </Button>
        </div>
      )}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-gutter">
        ...
        <Card className="lg:col-span-8 p-card-padding">
          <div className="flex justify-between items-center mb-6">
            ...
            <button onClick={() => router.push("/overhaul/tasks")} className="text-primary text-sm font-semibold hover:underline">
              전체 작업 보기
            </button>
          </div>
